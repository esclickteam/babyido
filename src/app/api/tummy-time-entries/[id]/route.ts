...
import { tummyTimeEndSchema } from "@/lib/validations/modules";
...
export async function DELETE(
  ...
  try {
    ...
    const entry = await TummyTimeEntry.findById(id).lean();
    if (!entry) {
    ...
    await TummyTimeEntry.findByIdAndDelete(id);
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = tummyTimeEndSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ error: "Invalid input" }, { status: 400 });
    }

    await connectDB();
    const entry = await TummyTimeEntry.findById(id);
    if (!entry) {
      return NextResponse.json({ error: "Not found" }, { status: 404 });
    }
